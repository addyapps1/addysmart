...
const CoinStatementSchema = new Schema({
  userID: { type: mongoose.Schema.Types.ObjectId, required: true, ref: "User" },
  transaction: {
    type: String,
    required: true,
    enum: ["mining", "withdrawal", "valuation"],
    default: "mining",
  },
  oldBalance: { type: Number, required: true, default: 0 },
  newBalance: { type: Number, required: true, default: 0 },
  withhdrawn: { type: Number, required: true, default: 0 },
  oldTBV_Coins: { type: Number, required: true, default: 0 },
  totalValuedCoins: { type: Number, required: true, default: 0 },
  valuedSum: { type: Number, required: true, default: 0 },
  valuedAt: { type: Number, required: true, default: 0 },
  newTBV_Coins: { type: Number, required: true, default: 0 },
  oldCoins: { type: Number, required: true, default: 0 },
  minedCoins: { type: Number, required: true, default: 0 },
  newCoins: { type: Number, required: true, default: 0 },
  month: { type: Number, required: true, default: 0 },
  yestTargetLoss: { type: Number, required: true, default: 0 },
  yestTargetTaskCompleted: { type: Number, required: true, default: 0 },
...
// Post hook
CoinStatementSchema.post("save", async function (doc, next) {
...
CoinStatementSchema.pre(/^find/, async function (next) {
...
CoinStatementSchema.post(/^find/, async function (docs, next) {
...
CoinStatementSchema.pre("aggregate", function (next) {
  this.pipeline().unshift({ $match: { releaseDate: { $lte: new Date() } } });
  next();
});

CoinStatementSchema.pre("save", async function (next) {
...
CoinStatementSchema.pre("save", function (next) {
...
const CoinStatement = model("CoinStatement", CoinStatementSchema);
