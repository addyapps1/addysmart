class Apifeatures {
  ...
  getRandom(numValues) {
    ...
    for (let i = this.query.length - 1; i > 0; i--) {
      ...
      [this.query[i], this.query[j]] = [this.query[j], this.query[i]]; // Swap elements
    }

    // Return the first numValues elements of the shuffled this.query
    this.query = this.query.slice(0, numValues);
    return this;
  }
  ////
