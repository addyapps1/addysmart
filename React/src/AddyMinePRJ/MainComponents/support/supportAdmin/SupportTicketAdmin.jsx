...
import SupportTicketsPresentation from "./SupportTicketPresentationAdmin";
...
const SupportTicketAdmin = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      let URL;
      // console.log("User.role.", getStoredUserObj().role);
      if (
        getStoredUserObj().role.includes("supreme") ||
        getStoredUserObj().role.includes("supportmanager")
      ) {
        URL = `${API_supportBase_url}api/a/v1.00/supportticket/agenthead`;
      } else {
        URL = `${API_supportBase_url}api/a/v1.00/supportticket/agent`;
      }
        ...
        supportTickets.map((ticket) => (
          <div key={ticket._id} className="ticket-item">
            <SupportTicketsPresentation
              ...
              onOpenTicket={() => navigate(`../ticketchatadmin/${ticket._id}`)}
            />
          </div>
        ))
      ) : (
